...
app.configure('production|development', function () {
    ...
    if (app.serverType !== 'master') {
        ...
        var areaIdMap = {};
        for (var id in areas) {
            areaIdMap[areas[id].area] = areas[id].id; // area id-->serverId
        }
        app.set('areaIdMap', areaIdMap); // 这样，每个服务器都存储的有这些地图的信息
    }
    ...
    app.route('area', routeUtil.area); // 路由配置
...
app.configure('production|development', 'manager', function () { // 添加或者减少服务器时的监听
...
app.configure('production|development', 'area|auth|connector|master', function () { // 数据库配置
...
app.configure('production|development', 'chat', function () {
    app.set('chatService', new ChatService(app)); // 由于只有一个聊天服务器，因此这是一个单例对象
	app.filter(pomelo.filters.serial());
	app.before(playerFilter());
